...
if "Données anonymisées" in etape1_option:
    ...
    etape2_choice = st.radio("La nouvelle finalité scientifique de recherche est-elle compatible avec la finalité initiale de la collecte ?", ["Oui", "Non"], index=0, key="r_e2")
    
    if etape2_choice == "Non":
        score -= 25
        actions_correctives.append("❌ **Action Étape 2 :** [LPD] Rupture du principe de finalité. Nouveau consentement requis ou mise à jour immédiate du cadre d'accord des participants.")
    ...
    etape3_choice = st.radio("Le consentement initial existant couvre-t-il expressément ou implicitement cette nouvelle finalité ?", ["Oui", "Non"], index=0, key="r_e3")
    
    if etape3_choice == "Non":
        score -= 25
        actions_correctives.append("❌ **Action Étape 3 :** [LPD] Défaut de couverture du consentement. Obligation d'obtenir un nouveau consentement écrit et exprès auprès des personnes concernées.")
    ...
    etape4_choice = st.radio("Avez-vous validé la présence de garanties suffisantes ou de Clauses Contractuelles Types (CCT) pour le stockage externe ?", ["Oui", "Non"], index=0, key="r_e4")
    ...
    if is_unil:
        ...
        unil_contract_choice = st.radio("Si un sous-traitant externe ou tiers manipule les données, un contrat de traitement conforme à la Directive UNIL 4.1 a-t-il été signé ?", ["Oui", "Non"], index=0, key="r_unil41")
        
        if unil_contract_choice == "Non":
            score -= 10
            actions_correctives.append("❌ **Gouvernance : [Directive UNIL 4.1]** Manquement de gouvernance contractuelle. Toute sous-traitance de données de recherche requiert la signature d'une convention de traitement standardisée validée par le **Service juridique de l'UNIL** avant le début des opérations informatiques.")

    # ==========================================
    # ÉTAPE 5 : SENSIBILITÉ DES DONNÉES (CORRIGÉE AVEC ÉNUMÉRATION TEXTUELLE)
    # ==========================================
    ...
    # Énumération textuelle intégrée sous forme de bloc informatif pour guider le chercheur
    st.markdown("""
    ...
    etape5_choice = st.radio(
        "Vos fichiers contiennent-ils au moins une de ces catégories particulières de données ?", 
        ["Oui", "Non"], 
        index=1, 
        key="r_e5"
    )
    ...
    if final_score == 100 and not etape5_sensitive:
        ...
        st.write("Le protocole s'aligne entièrement sur les exigences de base de la LPD, de la LPrD et du RNPD.")
    else:
        if final_score >= 70:
