...
# ==========================================
# ÉTAPE 1 : VÉRIFICATION INITIALE DES DONNÉES (CORRIGÉE AVEC TAXONOMIE DES DONNÉES)
# ==========================================
st.markdown('<div class="section-header">Étape 1 : Vérification des données (Art. 5 LPD)</div>', unsafe_allow_html=True)

# Bloc d'aide didactique pour clarifier la taxonomie des données selon la législation suisse
st.markdown("""
...
• <b>Données personnelles brutes :</b> Informations permettant d'identifier directement une personne (ex: nom, prénom, email, photo).<br>
• <b>Données pseudonymisées / codées :</b> Données dont les identifiants directs ont été remplacés par une clé ou un code. Elles restent juridiquement des données personnelles car il est possible de ré-identifier les personnes grâce à la clé de décodage.<br>
• <b>Données anonymisées :</b> Processus irréversible qui rend l'identification d'une personne définitivement impossible. Elles ne sont plus soumises à la LPD (réutilisation libre).
</div>
""", unsafe_allow_html=True)
...
if "Données anonymisées" in etape1_option:
    ...
    if is_unil:
        ...
        if unil_contract_choice == "Non":
            score -= 10
            actions_correctives.append(
                "❌ **Gouvernance : [Directive UNIL 4.1]** Manquement de gouvernance contractuelle. Toute sous-traitance de données de recherche requiert la signature d'une convention de traitement standardisée validée par le **Service juridique de l'UNIL** avant le début des opérations informatiques.")

    # ==========================================
    # ÉTAPE 5 : SENSIBILITÉ DES DONNÉES
    # ==========================================
    st.markdown('<div class="section-header">Étape 5 : Sensibilité des données (Art. 5 let. c LPD)</div>',
                unsafe_allow_html=True)
    ...
    etape5_choice = st.radio("Vos fichiers contiennent-ils au moins une de ces catégories particulières de données ?",
                             ["Oui", "Non"], index=1, key="r_e5")
    ...
    if final_score == 100 and not etape5_sensitive:
        ...
        if final_score >= 70:
            ...
            st.write("Le score is insuffisant. Le traitement informatique doit être suspendu jusqu'à correction.")
